...
# --- 3. Write Program IDs to JSON and .env files ---
echo "✍️  Writing program IDs to configuration files..."

# Create a JSON file for easy access
printf '{\n  "donation_pool": "%s",\n  "nft_tracker": "%s"\n}\n' "$DONATION_POOL_ID" "$NFT_TRACKER_ID" > contracts/program-ids.json
echo "   - Created contracts/program-ids.json"

# Update .env.example files
sed -i.bak -e '/^DONATION_POOL_PROGRAM_ID=/d' -e '/^NFT_TRACKER_PROGRAM_ID=/d' backend/.env.example
...
sed -i.bak -e '/^VITE_DONATION_POOL_PROGRAM_ID=/d' -e '/^VITE_NFT_TRACKER_PROGRAM_ID=/d' frontend/.env.example
...
rm -f backend/.env.example.bak frontend/.env.example.bak
